...
const Course = () => {
  ...
  const submitCourse = async () => {
    ...
    try {
      ...
      // Save domain information for each file
      for (const fileDetail of fileDetails) {
        try {
          await axios.post("https://aistudiumb-9jub.onrender.com/domains/add", {
            domainName: fileDetail.domain,
            subdomain: fileDetail.subdomain,
            document: fileDetail.fileUrl
          });
        } catch (error) {
          console.error("Error saving domain information:", error);
          toast.error(`Failed to save domain information for ${fileDetail.fileName}`);
        }
      }
